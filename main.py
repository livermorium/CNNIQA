...
def loss_fn(y_pred, y):
    return F.l1_loss(y_pred, y[0])  # 相比l2 对离群点、异常值 不敏感
...
class IQAPerformance(Metric):
    ...
    def compute(self):
        sq = np.reshape(np.asarray(self._y), (-1,))  # to 1 dim
        ...
        # pred-gt > 2 gt_std(图片主观质量分标准差（与均值的差) 的比例
...
def run(
    ...
    def log_validation_results(engine):
        ...
        print(
            "Validation Results - Epoch: {} SROCC: {:.4f} KROCC: {:.4f} PLCC: {:.4f} RMSE: {:.4f} MAE: {:.4f} OR: {:.2f}%".format(
                engine.state.epoch, SROCC, KROCC, PLCC, RMSE, MAE, 100 * OR
            )
        )
    ...
    def log_testing_results(engine):
        if config["test_ratio"] > 0 and config["test_during_training"]:
            ...
            SROCC, KROCC, PLCC, RMSE, MAE, OR = metrics["IQA_performance"]
            print(
                "Testing Results    - Epoch: {} SROCC: {:.4f} KROCC: {:.4f} PLCC: {:.4f} RMSE: {:.4f} MAE: {:.4f} OR: {:.2f}%".format(
                    engine.state.epoch, SROCC, KROCC, PLCC, RMSE, MAE, 100 * OR
                )
            )
    ...
    def final_testing_results(engine):
        if config["test_ratio"]:
            ...
            global best_epoch
            print(
                "Final Test Results - Epoch: {} SROCC: {:.4f} KROCC: {:.4f} PLCC: {:.4f} RMSE: {:.4f} MAE: {:.4f} OR: {:.2f}%".format(
                    best_epoch, SROCC, KROCC, PLCC, RMSE, MAE, 100 * OR
                )
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--exp_id", default="0", type=str, help="exp id (default: 0)")
    parser.add_argument(
        "--database", default="LIVE", type=str, help="database name (default: LIVE)"
    )
    parser.add_argument(
        "--model", default="CNNIQA", type=str, help="model name (default: CNNIQA)"
    )
    ...
    torch.backends.cudnn.deterministic = True  # 确定性卷积，相当于把所有操作的seed=0，以便重现，会变慢
    torch.backends.cudnn.benchmark = False  # 使用benchmark以启动CUDNN_FIND自动寻找最快的操作，当计算图不会改变的时候（每次输入形状相同，模型不改变）的情况下可以提高性能，反之则降低性能
    ...
    # like dict update, config[args.database] attrs -> config attrs
    config.update(config[args.database])
    config.update(config[args.model])

    log_dir = "{}/EXP{}-{}-{}-lr={}-{}".format(
        args.log_dir,
        args.exp_id,
        args.database,
        args.model,
    ...
    trained_model_file = "checkpoints/{}-{}-EXP{}-lr={}".format(
        args.model, args.database, args.exp_id, args.lr
    )
    ensure_dir("results")
    save_result_file = "results/{}-{}-EXP{}-lr={}".format(
        args.model, args.database, args.exp_id, args.lr
    )
